...
int k;
int i,j;
int node[30000][2],count1=0,key_set[11201][121][2],irr_poly[3],add_poly[3],mul_poly[3],vector[1000][2],poly_a[3],poly_b[3],poly_x[3];
...
//finding q for given n where k=(q*q)
   ...
   // fprintf(fp,"\nqt : %d\n",pt);
    printf("\n p=%d",p);
    printf("\n r=%d",r);
    k=m*m;                                          //According to Simonova's model k=m^2
    printf("\n Keys per node (k) :: %d",k);
    fprintf(fp,"\nkeys per node : %d",k);
    fprintf(fp,"\nr : %d",r);
    int x=r+1;
...
//print keys
    ...
    for(a1=0;a1<no_of_node;a1++)
    {
        ...
        for(a2=0;a2<k;a2++)
        {
            ...
            {
                if((compromised_keys[a3][0]==key_set[a1][a2][0])&&(compromised_keys[a3][1]==key_set[a1][a2][1]))
                {
                    flag1=1;
                    continue;
                }
    ...
    vs=(float)flagvs/(no_of_node-compromised_node);
    //printf("\nV(s)=%f\n",vs);*/
