...
//                printf("k(%d,%d)(%d,%d)  ",key_set[a4*p+a5][a6][0],key_set[a4*p+a5][a6][1],key_set[(a4+1)*p+(a5-1)][a6][0],key_set[(a4+1)*p+(a5-1)][a6][1]);
        ...
        for(a7=0; a7<k; a7++)
        {
            merge_key_set[merge_k][a7][0]=key_set[a4*p+0][a7][0];
            merge_key_set[merge_k][a7][1]=key_set[a4*p+0][a7][1];

            merge_key_set[merge_k][a7+k][0]=key_set[(a4+1)*p+(p-1)][a7][0];
            merge_key_set[merge_k][a7+k][1]=key_set[(a4+1)*p+(p-1)][a7][1];
            //printf("(%d,%d)  (%d,%d)",key_set[a4*p+0][a7][0],key_set[a4*p+0][a7][1],key_set[(a4+1)*p+(p-1)][a7][0],key_set[(a4+1)*p+(p-1)][a7][1]);
        }
        merge_k++;
        printf("\n<(%d,%d)  (%d,%d)>",a4,0,a4+1,p-1);
    }
...
//merging last 3X3
    ...
    printf("%d\n",merge_k);
    //printf("((%d,%d))",merge_key_set[7][1][0],merge_key_set[7][1][1]);

    for(a6=0;a6<merge_k;a6++)
    {
        printf("M(%d)::",a6);
        for(a7=0;a7<2*k;a7++)
        {
            printf("(%d,%d)",merge_key_set[a6][a7][0],merge_key_set[a6][a7][1]);
        }
        printf("\n");
    }
}
        ...
        for(a1=0; a1<p; a1++)  //a:a1 b:a2 x:a3 key(x,ax+b mod p)
        {
            for(a2=0; a2<p; a2++)
            {
                for(a3=0; a3<k; a3++)
                {
                    ...
                    key_set[a1*p+a2][a3][1]=((a1*(a3%p))+a2)%p;
                }
            }
        }
    }
        ...
        for(a1=0; a1<p; a1++)  //a:a1 b:a2 x:a3 key(x,ax+b mod p)
        {
            for(a2=0; a2<p; a2++)
            {
                for(a3=0; a3<k; a3++)
                {
                    ...
                    poly_x[0]=vector[(a3%p)][0];
                    poly_x[1]=vector[(a3%p)][1];
